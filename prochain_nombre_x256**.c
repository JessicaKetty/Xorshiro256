...
void copie (int l, int c, mat * xo,mat * m)
{
    int k=0,o;
    for (int i=l; i<l+64; i++)
    {
        o=0;
        for (int j=c; j<c+64; j++)
        {
            xo->m[i][j]=m->m[k][o];
            o++;
        }
...
mat * inv_matrice_xoshiro_256ss(void)
{
    ...
    copie(128, 0, xo, p(s,p(sr_1, r_1)));
    copie(128, 64, xo, p(sr_1,I));
    copie(128, 128, xo, r_1I);
    copie(128, 192, xo, r_1);

    copie(192, 0, xo,p(sr_1, r_1I));
    copie(192, 64, xo, sr_1);
    copie(192, 128, xo, r_1);
    copie(192, 192, xo, r_1);
    return xo;
}
...
uint64_t prochain_nombre (uint64_t a, uint64_t b, uint64_t c, uint64_t d)
{
    ...
    mat *xochirof=matrice_xoshiro_256ss();
    //mat * x=GaussEliminationAlgo(xochirof, v);
    mat *x=mult(inv_matrice_xoshiro_256ss(),v);
